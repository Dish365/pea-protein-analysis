...
def calculate_utility_costs(utilities: List[Dict[str, float]]) -> Dict[str, float]:
    ...
    for utility in utilities:
        ...
        if not all(field in utility for field in required_fields):
            raise ValueError(
                f"Utility must contain all required fields: {required_fields}"
            )

        # Validate numeric values
        if utility["consumption"] < 0:
            raise ValueError(
                f"Consumption cannot be negative for utility: {utility.get('name', 'unknown')}"
            )

        if utility["unit_price"] < 0:
            raise ValueError(
                f"Unit price cannot be negative for utility: {utility.get('name', 'unknown')}"
            )
