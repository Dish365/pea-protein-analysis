...
class SeparationEfficiencyInput(BaseModel):
    ...
    mass_flow: Dict[str, float] = Field(..., description="Input and output mass flows")
    process_data: Optional[List[Dict]] = Field(
        None, description="Step-wise process data"
    )
    target_purity: Optional[float] = Field(
        None, description="Target protein purity percentage"
    )
...
class ParticleSizeInput(BaseModel):
    ...
    weights: Optional[List[float]] = Field(
        None, description="Optional weights for each particle size"
    )
