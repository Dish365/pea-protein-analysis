...
# Load Rust library
lib_path = Path(__file__).parent.parent.parent.parent / "backend/rust_modules/target/release/libmatrix_ops.so"
...
class EnvironmentalIntegrator:
    ...
    def calculate_impact_matrix(
        self,
        process_matrix: List[List[float]],
        impact_factors: List[List[float]]
    ) -> List[List[float]]:
    ...
    def calculate_allocation_factors(
        ...
        matrix = (ctypes.c_double * (n * n))(*[x for row in correlation_matrix for x in row])
