...
class IndexManager:
    ...
    def analyze_table_indexes(self, model: models.Model) -> Dict[str, Any]:
        ...
        recommendations = self._generate_index_recommendations(
            model, existing_indexes, index_usage
        )
    ...
    def _get_existing_indexes(self, model: models.Model) -> List[Dict[str, Any]]:
        ...
        for idx in meta.indexes:
            indexes.append({"name": idx.name, "fields": idx.fields, "type": "explicit"})

        # Get unique constraints
        for constraint in meta.unique_constraints:
            indexes.append(
                {"name": constraint.name, "fields": constraint.fields, "type": "unique"}
            )
    ...
    def _analyze_query_patterns(self, model: models.Model) -> List[Dict[str, Any]]:
        ...
        for field in meta.fields:
            if isinstance(field, models.ForeignKey):
                patterns.append(
                    {"fields": [field.name], "frequency": "high", "type": "foreign_key"}
                )

        return patterns

    def _has_matching_index(
        self, fields: List[str], existing_indexes: List[Dict[str, Any]]
    ) -> bool:
