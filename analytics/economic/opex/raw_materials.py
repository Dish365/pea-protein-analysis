...
def calculate_raw_material_costs(materials: List[Dict[str, float]]) -> Dict[str, float]:
    ...
    for material in materials:
        ...
        if material['quantity'] < 0:
            raise ValueError(f"Quantity cannot be negative for material: {material.get('name', 'unknown')}")
            
        if material['unit_price'] < 0:
            raise ValueError(f"Unit price cannot be negative for material: {material.get('name', 'unknown')}")
