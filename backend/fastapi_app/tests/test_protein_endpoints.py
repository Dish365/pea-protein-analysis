...
def test_protein_recovery(client, protein_recovery_data):
    response = client.post(
        "/process/technical/protein-recovery/", json=protein_recovery_data
    )
...
def test_separation_efficiency(client, separation_efficiency_data):
    response = client.post(
        "/process/technical/separation-efficiency/", json=separation_efficiency_data
    )
    assert response.status_code == 200
...
def test_invalid_separation_efficiency_input(client):
    ...
    response = client.post(
        "/process/technical/separation-efficiency/", json=invalid_data
    )
    assert response.status_code == 422  # Validation error
