...
def estimate_total_investment(equipment_costs: float, installation_costs: float, indirect_costs: float) -> Dict[str, float]:
    """
...
def estimate_annual_costs(
    ...
    capital_recovery_factor = (interest_rate * (1 + interest_rate)**project_years) / ((1 + interest_rate)**project_years - 1)
